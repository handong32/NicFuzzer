...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if float(tput) == -1.0:
            ...
            reward = float(tput) - prev_tput
        
        print("i_episode=%d t=%d RXD=%d TPUT=%.2f REWARD=%.2f" % (i_episode, t, state, float(tput), float(reward)))
    ...
    if i_episode % TARGET_UPDATE == 0:
        ...
        target_net.load_state_dict(policy_net.state_dict())
        torch.save(target_net.state_dict(), "target_net910.pt")
        torch.save(policy_net.state_dict(), "policy_net910.pt")
