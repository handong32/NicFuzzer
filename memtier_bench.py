...
def main(rxu):
    ...
    runRemoteCommand("chrt -r 1 perf stat -C 1,3,5,7,9,11,13,15 -D 1000 -o perf.out -e power/energy-pkg/,power/energy-ram/ -x, numactl --cpunodebind=1 --membind=1 memcached -u nobody -t 8 -m 64G -l "+MASTER+" -B binary")
    time.sleep(1)
    ...
    ## normal distribution?
    runMemtier("chrt -r 1 numactl --cpunodebind=1 --membind=1 memtier_benchmark -s "+MASTER+" -p 11211 -t 8 --random-data --data-size-range=4-204 --data-size-pattern=S --key-minimum=200 --key-maximum=400 --ratio=1:1 --pipeline=16 -P memcache_binary --test-time=28 --json-out-file=memtier.json")
    e = datetime.now()
